...
const sendSolution = (year, day, part, solution) => {
  ...
  fetch(`https://adventofcode.com/${year}/day/${day}/answer`, {
    ...
    const info = new JSDOM(body).window.document.querySelector("main").textContent.replace(/\[.*\]/, "").trim();
...
Status: PART ${part} SOLVED!`);
    ...
    if (waitStr !== null || waitNum !== null) {
      ...
      const delayStr = `${waitTime.d > 0 ? `${waitTime.d}d ` : ""}${waitTime.h > 0 ? `${waitTime.h}h ` : ""}${waitTime.m > 0 ? `${waitTime.m}m ` : ""}${waitTime.s > 0 ? `${waitTime.s}s` : ""}`;
